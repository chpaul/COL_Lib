...
namespace COL.MassLib
{
    class HCDScoring
    {
        double mdbl_max_mz = 700;
        double mdbl_min_mz = 0;
        int mint_min_number_peaks = 2;
        float[] marr_theoretical_peaks = new float[7];
        float[] marr_theoretical_hm = new float[7];
        float[] marr_theoretical_ca = new float[7];
        float[] marr_theoretical_cs = new float[7];
        float[] marr_theoretical_hybrid = new float[7];
        List<int> identified_peak_indices;
        public HCDScoring()
        {
            marr_theoretical_peaks[6] = 657.23f; // NeuAc + Gal/Mannose + GlcNac
            marr_theoretical_peaks[5] = 366.14f; // GlcNAc + Gal/Mannose
            marr_theoretical_peaks[4] = 292.1f; //NeuAc
            marr_theoretical_peaks[3] = 274.09f; // NeuAc-water loss
            marr_theoretical_peaks[2] = 204.09f; // GlcNAc
            marr_theoretical_peaks[1] = 163.06f; // Gal/Mannose //prev 168.06
            marr_theoretical_peaks[0] = 138.05f; // Oxonium ion at HexNac

            //Distribution of intensities for hm
            marr_theoretical_hm[0] = 0f;
            marr_theoretical_hm[1] = 0.99f;
            marr_theoretical_hm[2] = 0f;
            marr_theoretical_hm[3] = 0f;
            marr_theoretical_hm[4] = 0f;
            marr_theoretical_hm[5] = 0.3f;
            marr_theoretical_hm[6] = 0f;

            //Distribution of intensities for ca
            marr_theoretical_ca[0] = 0;
            marr_theoretical_ca[1] = 0.3f;
            marr_theoretical_ca[2] = 0.5f;
            marr_theoretical_ca[3] = 0;
            marr_theoretical_ca[4] = 0;
            marr_theoretical_ca[5] = 0.99f;
            marr_theoretical_ca[6] = 0;

            //Distribution of intensities for cs
            marr_theoretical_cs[0] = 0.7f;
            marr_theoretical_cs[1] = 0.7f;
            marr_theoretical_cs[2] = 0.7f;
            marr_theoretical_cs[3] = 0.7f;
            marr_theoretical_cs[4] = 0.8f;
            marr_theoretical_cs[5] = 0.7f;
            marr_theoretical_cs[6] = 0.7f;

            //Distribution of intensities for hybrid
            marr_theoretical_hybrid[0] = 0.6f;
            marr_theoretical_hybrid[1] = 0.99f;
            marr_theoretical_hybrid[2] = 0.6f;
            marr_theoretical_hybrid[3] = 0.6f;
            marr_theoretical_hybrid[4] = 0.6f;
            marr_theoretical_hybrid[5] = 0.6f;
            marr_theoretical_hybrid[6] = 0.6f;

            // CC Mass
            float mdbl_cc_mass = Atoms.ProtonMass;
            identified_peak_indices = new List<int>();
        }
        ...
        private long Factorial(int argX)
        {
            int counter = argX;
        ...
        private float Correlation(float[] argX, float[] argY, int argDelay)
		{
			int i,j ;
			...
			for (i=0;i<n;i++) 
			{
				  sx += (argX[i] - mx) * (argX[i] - mx);
				  sy += (argY[i] - my) * (argY[i] - my);
		   }
